...
public class Task {
	...
	public static String Withdraw(Transaction b, java.sql.Connection con, java.sql.Statement st, boolean trensfer) {
		...
		if (isIt == 10) {
			...
			boolean acState = Chech_acc_state(accState);
			if (acState) {
				...
				if (BalLimit) {
					...
					if (BalCHECK) {
						boolean limitDay = Chech_limit_Req_perday(AccId, con, st);
						if (limitDay) {
							...
							if (Balbasket) {
								...
								if (widState) {
									...
									Team.Basket_money = Team.Basket_money + amount;
									Val = "Withdraw_inDB failed sender";
								}
							} else {
								Val = "mojody EXCEED basket sender " + amount + " > " + Team.Basket_money;
							}
						} else {
		...
		Update_Transaction_Result_DB(Val, transactionId, con, st);
		return Val;
	}
	...
	public static synchronized Boolean ChechMoneyInBasket(long amo) {
		if (amo <= Team.Basket_money) {
			Team.Basket_money = Team.Basket_money - amo;
			return true;
		}
		return false;
	...
	public static synchronized Boolean Chech_limit_Req_perday(Long id, java.sql.Connection con, java.sql.Statement st) {
		boolean lim = false;
		try {
			String query_read = "SELECT * FROM `account` WHERE `accountNumber`=%s";
			query_read = String.format(query_read, id);
	...
	public static synchronized Boolean Chech_acc_state(int state) {
		if (state == 0)
			return true;
		return false;
	}
	...
	public static synchronized void Update_Transaction_Result_DB(String value, long id, java.sql.Connection con,
			java.sql.Statement st) {
